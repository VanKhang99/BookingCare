...
const CarouselDoctorClinic = ({ clinicId, settings }) => {
    ...
    <Carousel className="slides" {...settings}>
      {doctorsById?.length > 0 &&
        doctorsById.map((doctor) => {
          ...
          return (
            <Link
              to={`/${path.CLINIC}/${clinicId}/${path.DOCTOR}/${doctorId}`}
              key={doctorId}
              className="slide"
            >
              <div className="slide-content">
                <div className="slide-content__img">
                  <img src={imageUrl} alt={fullName} style={{}} />
                </div>
                <div className="slide-info">
                  <div className="slide-info__name">{fullName}</div>
...
export default CarouselDoctorClinic;
